...
- [x] The interactive validation HUD now exposes the actual selected Source gains for `Direct`, `Early Reflection`, and `Late Reverb`; the Rendered label is `DIRECT+EARLY+LATE`.
- [x] Interactive smoke now requires finite, non-zero `OverallGain`, `IndirectGain`, `EarlyReflectionGain`, and `LateReverbGain`, plus `IndirectMode=HybridReverb`, after the F3 Rendered toggle. The parser rejects `rendered_components=missing`.
- [x] RED/green parser coverage was added for the missing rendered-component marker. Full Python validation passed `62/62`; realtime audit passed `39 functions / 40 bodies / 1775 lines` with all forbidden-operation counters `0`.
- [x] Prescribed `uv run script\\build_and_validate.py` exited `0` with project and standalone plugin build `49/49` actions. Exact interactive launcher exited `0`; smoke marker recorded `moved_cm=50.998`, listener/camera/origin errors `0 cm`, `realtime/baked/hybrid=1/1/1`, `A/B=1/1`, `f3_source_preserved=1`, `rendered_components=direct_early_late`, and no foreign audio. Game log: `D:\\Labs\\2602-unreal\\ue-audio-plugin\\TestProject\\UeVersion1\\Saved\\Logs\\UERayTracingAudioValidation-Game-1785660205992184900.log`.
- [x] Exact default launcher exited `0`; Game evidence reported hardware/CPU paths `171/171`, gain `0.001625/0.001625`, Direct sweep `206` generations, zero dropout, restored/hardware `1/1`, all data sources passed, and hard realtime `1584/0/0`. Game log: `D:\\Labs\\2602-unreal\\ue-audio-plugin\\TestProject\\UeVersion1\\Saved\\Logs\\UERayTracingAudioValidation-Game-1785660431487845600.log`; Editor log: `D:\\Labs\\2602-unreal\\ue-audio-plugin\\TestProject\\UeVersion1\\Saved\\Logs\\UERayTracingAudioValidation-Editor-1785660612831773000.log`.
- [ ] Human PIE listening, click/pop and audible-quality judgment, moving-player/moving-occlusion listening, true multi-PIE hardware isolation, target-device R3 listening, and the pre-existing deferred ledger remain open; these gates cannot be inferred from the new runtime marker.
